...
2026-09-15 terrain v2: 7651 tiles (101 across), 5 recursive aperture-7 levels, octave ladder generalized (x3 per level), contrast gamma; single fine octave collapses to 0% at top vs 20-29% (γ=1) / 33-47% (γ=2) / 40-60% (γ=3); basins L1 5 → L2 14 → L3 114 tiles; R=100 verified headless
2026-09-15 ascent shipped: docs/ascent.html — L1 r=50 world with material economy; Seal & ascend characterizes the world through six gates (15 CG solves, 1.1 s at 7,651 nodes) and reveals an r=50 world of worlds with the same fractal field (origin richness = exact mean of L1 tiles); copies yield ∝ local/origin richness; L2 basins form; L2 income ~10× L1
2026-09-18 ascent II shipped: two richness fluids (A local, B regional), four rim springs draining to a central lake, streamline advection of leached A/B, 7-tile graded reactors (18/35/60% from the Contracts greybox) making C only where both streams pass, conduits/dams for routing, raw A/B collected at the lake, seal priced in C, L2 copies yield A/B/C scaled by local richness
